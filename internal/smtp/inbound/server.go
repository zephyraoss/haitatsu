...
func New(cfg config.SMTPConfig, domain string, tlsConfig *tls.Config, resolver *routing.Resolver, messages *messages.Service) *Server {
	server := smtp.NewServer(&backend{resolver: resolver, messages: messages})
...
func (b *backend) NewSession(*smtp.Conn) (smtp.Session, error) {
	return &session{resolver: b.resolver, messages: b.messages}, nil
}
